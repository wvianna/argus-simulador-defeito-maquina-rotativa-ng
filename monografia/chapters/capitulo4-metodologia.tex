...
\begin{enumerate}
  ...
  de aceitação (CA-001 a CA-014) em `SPECIFICATION.md`, com matriz de
  rastreabilidade.
  \item \textbf{Design}: arquitetura, contratos de API, modelo de dados e
  detalhamento por tarefas pequenas (T-001 a T-013) em `.specs/`.
...
\bottomrule
\end{tabular}
\caption*{\footnotesize Fonte: `frontend/package.json`, `backend/requirements*.txt` e `docker-compose.yml` do projeto.}
\end{table}
...
\begin{enumerate}
  ...
  (`signal_generator`, `fft_processor`, `discard_engine`), sem acesso a banco
  ...
  contra um PostgreSQL real provisório (contêiner via `testcontainers`),
  ...
  contra a aplicação FastAPI, verificando os contratos de `POST /simulacoes`,
  `POST /snapshots` e `GET /health`, incluindo validações de entrada e a
  violação de Nyquist.
...
\begin{enumerate}
  \item subir o ambiente completo com `start.sh` (Docker Compose: banco,
