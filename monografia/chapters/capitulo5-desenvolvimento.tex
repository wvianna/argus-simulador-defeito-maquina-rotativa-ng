...
\texttt{backend/} e \texttt{frontend/}.
...
\par\vspace{2pt}{\footnotesize Fonte: elaboração própria, com base na arquitetura real do projeto.}
\end{figure}

O fluxo nominal de uma simulação é: (i) a interface envia os parâmetros a
\texttt{POST /simulacoes}; (ii) o back-end valida a entrada (incluindo o critério de
...
\midrule
\texttt{signal\_generator} & FR-001, FR-002, FR-003 \\
\texttt{fft\_processor} & FR-004, FR-005, FR-006, FR-019, NFR-001 \\
\texttt{discard\_engine} & FR-007, FR-008, FR-009, FR-010, FR-011 \\
\texttt{persistence} & FR-011, FR-012, FR-013, NFR-003, NFR-004 \\
\texttt{api} & FR-016, FR-017, FR-018, NFR-002 \\
\texttt{frontend} & FR-014, FR-015, FR-016, FR-017, FR-018, FR-019 \\
\bottomrule
\end{tabular}
\par\vspace{2pt}{\footnotesize Fonte: SPECIFICATION.md do projeto.}
\end{table}

\section{Geração de sinais sintéticos}

O módulo \texttt{signal\_generator} implementa a síntese no domínio do tempo. Para cada
...
\bottomrule
\end{tabular}
\par\vspace{2pt}{\footnotesize Fonte: \texttt{DEFECT\_PROFILES} e \texttt{\_componentes\_do\_defeito} em \texttt{backend/app/domain/signal\_generator.py}.}
\end{table}
...
O módulo \texttt{fft\_processor} calcula a FFT real (via \texttt{numpy.fft.rfft}), extrai os
...
O módulo \texttt{discard\_engine} implementa a regra de ouro e o Paralelepípedo de
...
última leitura efetivamente persistida do ponto (ou \texttt{None} na primeira) e as
...
A camada \texttt{persistence} utiliza SQLAlchemy 2.x assíncrono e segue a hierarquia
...
documento JSON em coluna \texttt{JSONB}, priorizando a velocidade de ingestão em
...
pontos & nó de medição; \texttt{ultima\_leitura\_persistida\_id} (ponteiro crítico) \\
leituras\_persistidas & picos\_r3 (JSONB), rms\_total/ruido/picos, valor\_dc, rotacao, níveis, timestamp \\
leituras\_trash & leitura descartada + \texttt{motivo\_descarte} \\
snapshots\_defeito & par sensor + tipo de defeito + vínculo com a leitura \\
\bottomrule
\end{tabular}
\par\vspace{2pt}{\footnotesize Fonte: \texttt{.specs/codebase/ARCHITECTURE.md} e migração Alembic do projeto.}
\end{table}

A primeira migração Alembic cria as oito relações da
Tabela~\ref{tab:schema}. O estado compartilhado crítico é o ponteiro
\texttt{pontos.ultima\_leitura\_persistida\_id}: a decisão de descarte lê esse ponteiro
...
escritas em \texttt{leituras\_persistidas} e atualizam o ponteiro; leituras descartadas
vão para \texttt{leituras\_trash}, sem alterar a referência.
...
\bottomrule
\end{tabular}
\par\vspace{2pt}{\footnotesize Fonte: \texttt{backend/app/api/} do projeto.}
\end{table}

A resposta de \texttt{POST /simulacoes} contém \texttt{sinal\_tempo} (decimado), \texttt{picos\_r3},
\texttt{rms\_total}, \texttt{rms\_ruido}, \texttt{rms\_picos}, \texttt{valor\_dc}, \texttt{rotacao}, \texttt{limiar\_picos} e
\texttt{limiar\_amplitude}, a decisão (\texttt{descartada} e \texttt{motivo\_descarte}),
\texttt{tempo\_processamento\_ms} e \texttt{taxa\_descarte\_acumulada}. Erros são mapeados como
\texttt{422} (entrada inválida ou violação de Nyquist), \texttt{404} (ponto inexistente) e
\texttt{500} (falha interna de persistência, sem corromper o ponteiro de referência).

\section{Front-end}

O front-end é uma SPA em React com gráficos Recharts, organizada em
componentes: \texttt{PainelSimulacao} (formulário), \texttt{PainelSinal} (sinal e RMS),
\texttt{GraficoFFT} (barras de espectro), \texttt{IndicadoresSimulacao} (tempo e taxa de
descarte), \texttt{ChecklistValidacao}, \texttt{SnapshotDefeito} e o componente \texttt{Help}
...
defeito simulado e persiste via \texttt{POST /snapshots} (FR-016). O client HTTP
...
\texttt{db} (PostgreSQL 16 com volume persistente), \texttt{backend} (imagem Python que executa
\texttt{alembic upgrade head} e inicia o Uvicorn na porta 8000) e \texttt{frontend} (build de
produção servido por nginx na porta 80, com proxy de \texttt{/api/} para o back-end).
Os scripts \texttt{start.sh} e \texttt{stop.sh} automatizam, respectivamente, o setup (checagem
do Docker, cópia do \texttt{.env}, build de imagem se ausente, subida com espera de
